...
export default function ReservationScreen({ route }) {
	...
	useEffect(() => {
		if (reservationAccepted) {
			Alert.alert("Accepted", "Your reservation request has been accepted.", [
				...
				const unsubscribeResStatus = onSnapshot(resStatusQuery, (snapshot) => {
					snapshot.docChanges().forEach((change) => {
						if (change.type === "added" || change.type === "modified") {
							...
							const message = `Reservation Status for Slot ${resStatusData.slotId + 1} is ${
								resStatusData.resStatus
							}`;
					...
					},
					{
						...
						onPress: () => {
							setReservedSlots((prevSlots) =>
								prevSlots.filter(
									(slot) =>
										slot.slotNumber !== slotNumber || slot.managementName !== item.managementName
								)
							);
	...
	const handleCancelReservation = async () => {
		const reservedSlot = reservedSlots.find(
			(slot) => slot.slotNumber === selectedSlot && slot.managementName === item.managementName
		);
			...
			<ScrollView contentContainerStyle={styles.scrollContainer}>
				...
				<Image
					...
					style={[
						...
						{ borderTopLeftRadius: 80, marginTop: 100, borderTopRightRadius: 80 },
					]}
				/>
				<Image
						...
						{reservedSlots.map((reservedSlot, index) => (
							<View key={index} style={styles.reservedSlotButton}>
								<Text style={styles.infoText}>
									Slot {reservedSlot.slotNumber} at {reservedSlot.managementName}
								</Text>
